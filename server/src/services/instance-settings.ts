...
export function instanceSettingsService(db: Db, options: InstanceSettingsServiceOptions = {}) {
  ...
  return {
    ...
    updateGeneral: async (patch: PatchInstanceGeneralSettings): Promise<InstanceSettings> => {
      ...
      const mergedGeneral = normalizeGeneralSettings({
        ...storedGeneral,
        ...patch,
        branding:
          patch.branding !== undefined
            ? { ...(storedGeneral.branding ?? DEFAULT_INSTANCE_BRANDING), ...patch.branding }
            : storedGeneral.branding,
        theming:
          patch.theming !== undefined
            ? { ...(storedGeneral.theming ?? DEFAULT_INSTANCE_THEMING), ...patch.theming }
            : storedGeneral.theming,
        navigation:
          patch.navigation !== undefined
            ? { ...(storedGeneral.navigation ?? DEFAULT_INSTANCE_NAVIGATION), ...patch.navigation }
            : storedGeneral.navigation,
        companyPermissions:
          patch.companyPermissions !== undefined
            ? {
                ...(storedGeneral.companyPermissions ?? DEFAULT_INSTANCE_COMPANY_PERMISSIONS),
                ...patch.companyPermissions,
              }
            : storedGeneral.companyPermissions,
      });
