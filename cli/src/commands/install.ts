  ...
  try {
    ...
    for (const [index, workspacePackage] of workspacePackages.entries()) {
      ...
      if (bundledDependencies.length > 0) {
        ...
        const realStagedPackage = fs.existsSync(stagedPackage) ? fs.realpathSync(stagedPackage) : stagedPackage;
        const realStagingRoot = fs.existsSync(stagingRoot) ? fs.realpathSync(stagingRoot) : stagingRoot;
        await runCommand("npm", ["pack", "--pack-destination", realStagingRoot], { cwd: realStagedPackage, env: buildEnv(), maxBuffer: 16 * 1024 * 1024 });
      } else {
